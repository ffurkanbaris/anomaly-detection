...
The project is designed for scenarios where anomalous or attack samples are scarce or unavailable during training, learning from benign (normal) data.
...
│
├── cnn-drocclf.py                        # Image-based (CNN) DROCC-LF training and evaluation script
├── mlp-drocc.py                          # Vector-based (MLP) DROCC-LF training and evaluation script
...
*   **Data Format:** Numerical vectors scaled to `[0, 1]` inside the dataset class.
...
*   **Target Scripts:** `cnn-drocclf.py` and `deepsvdd_baseline.py`
...
*   **WUSTL-EHMS (Label Encoding):**
    ...
    ```
    *Performs feature selection, log scaling for numerics, and deterministic label encoding for categorical variables.*
*   **WUSTL-EHMS (One-Hot Encoding):**
    ```bash
    python utils/data_preprocessing_onehot_encode.py
    ```
    *Uses One-Hot encoding to produce 36 features, suitable for 6x6 pixels.*
*   **CIC Network Traffic:**
    ```bash
...
python mlp-drocc.py --epochs 10 --lr 0.001 --model_dir log_drocc_vector

# Evaluate the saved MLP model
python mlp-drocc.py --eval 1 --model_dir log_drocc_vector
```
...
python cnn-drocclf.py --epochs 20 --batch_size 128 --lr 0.001 --model_dir log_drocc

# Evaluate the saved CNN model
python cnn-drocclf.py --eval 1 --model_dir log_drocc
```
...
*   **ROC-AUC Score:** Evaluates the model quality independently of the threshold.
*   **Precision & Recall (@FPR 3% and 5%):** Precision/recall at specific false positive rates.
*   **Confusion Matrix:** Breakdown of True Positives, False Positives, True Negatives, and False Negatives.
*   **Youden's J Threshold:** Determines the optimal score cut-off on the ROC curve.
...
2.  `evaluation_distribution_cm.png`: Score distribution histograms, target FPR decision boundaries, Youden's J index lines, and confusion matrices.
